...
function Install-FreeClaudeCode {
    ...
    # Automatically bind uv tool to a whitelisted Python installation
    # to prevent Windows Application Control from blocking execution.
    $whitelistedPython = $null
    if (Test-Path "C:\Users\soura\anaconda3\python.exe") {
        $whitelistedPython = "C:\Users\soura\anaconda3\python.exe"
    } else {
        # Check C:\Program Files system Python folders
        ...
        foreach ($folder in $folders) {
            ...
            if (Test-Path $exe) {
                $whitelistedPython = $exe
                break
            }
        }
    }

    if ($whitelistedPython) {
        Write-Host "Binding uv tool installation to whitelisted Python: $whitelistedPython" -ForegroundColor Green
        $toolArgs += @("--python", $whitelistedPython)
    }
